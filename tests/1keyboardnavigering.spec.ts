...
test.describe('Keyboard navigering', () => {
  test('Vi navigerer forsiden med mange elementer', async ({ page }) => {
    ...
    await expect(page.locator('text=Du har en ny søknad om sykepenger')).toHaveCSS('background-color', 'rgba(0, 0, 0, 0)');
...
// );
       ...
       await expect(focusedElement).toContainText(
      'Hogwarts School of Witchcraft and Wizardry',
    ); // toHaveText får også med vis mer som kommer fra expand knappen


    await page.keyboard.press('Space');
    await expect(
      page.locator(
        'text=Arbeidsgiveren har meldt inn at Albus Dumbledore skal følge deg opp mens du er syk.',
      ),
    ).toBeVisible();
  ...
  test('Vi navigerer forsiden med lenke til mangelnde inntektsmelding', async ({ page }) => {
    ...
    await expect(
      page.locator('text=Vi har varslet arbeidsgiveren din om dette'),
    ).toBeVisible();
  });
});
