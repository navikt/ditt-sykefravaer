...
import * as dayjs from 'dayjs'; // there are other solutions: https://chatgpt.com/c/67065cc3-d988-8001-9b5a-38ed4bf52b43
import { tilLesbarDatoMedArstall } from '../src/utils/dato-utils';

test.describe('Maksdato', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(
      'http://localhost:8080/syk/sykefravaer?testperson=syk-naa-med-maksdato'
    );
  });

  test('Finner maksdato kortet og kan åpne den', async ({ page }) => {
    const sisteUtbetaling = tilLesbarDatoMedArstall(
      dayjs().subtract(1, 'day').toDate()
    );
    const maksdato = tilLesbarDatoMedArstall(
      dayjs().add(340, 'days').toDate()
    );
    const forventetTekst = `Maksdato per ${sisteUtbetaling} er ${maksdato}`;

    await page
      .getByRole('region', { name: 'Beregnet slutt på sykepenger' })
      .click();
    await expect(
  ...
  test('Innholdet i maksdato kortet er riktig', async ({ page }) => {
    ...
    // Sjekker lenker
    await expect(
      page.locator('text=folketrygdloven § 8-12.')
    ).toHaveAttribute(
      'href',
      'https://lovdata.no/nav/folketrygdloven/kap8/%C2%A78-12'
    );
    await expect(
      page.locator('text=Det er egne regler for deg som er mellom 67 og 70 år.')
    ).toHaveAttribute(
      'href',
    ...
    await expect(
      page.locator(
        'text=Les mer om mulighetene dine etter det er slutt på sykepengene.'
      )
    ).toHaveAttribute(
      'href',
      'https://www.nav.no/sykepenger#hvor-lenge'
