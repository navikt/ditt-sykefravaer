describe('Tester oppfølgingsplan oppgaver', () => {
    ...
    it('En til godkjenning', () => {
        ...
        cy.get('.oppgaver > .navds-alert').contains('Du har en oppfølgingsplan som venter på godkjenning av deg')
    })

    it('To til godkjenning', () => {
        cy.visit('http://localhost:8080/syk/sykefravaer?testperson=to-nye-oppfolgingsplaner-til-godkjenning')
        cy.get('.oppgaver > .navds-alert').contains('Du har to oppfølgingsplaner som venter på godkjenning av deg')
    })
})
