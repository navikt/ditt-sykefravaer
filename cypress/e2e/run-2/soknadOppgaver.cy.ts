describe('Tester at oppgaver vises for søknader', () => {
    it('En vanlig søknad for arbeidstaker', () => {
        ...
        cy.get('[data-cy="oppgaver"]').get(' .navds-alert').contains('Du har en ny søknad om sykepenger')
    })

    it('To nye søknader', () => {
        cy.visit('http://localhost:8080/syk/sykefravaer?testperson=flere-soknader')
        cy.get('[data-cy="oppgaver"]').get(' .navds-alert').contains('Du har to nye søknader om sykepenger')
    })
