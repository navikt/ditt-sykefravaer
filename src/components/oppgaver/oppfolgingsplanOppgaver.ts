...
const erOppfolgingsdialogKnyttetTilGyldigSykmelding = (oppfolgingsdialog: Oppfolgingsplan, sykmeldinger: Sykmelding[]) => {
    const dagensDato = new Date()
    return sykmeldinger.filter((sykmelding) => {
        return oppfolgingsdialog.virksomhet.virksomhetsnummer === sykmelding.sykmeldingStatus.arbeidsgiver?.orgnummer
...
export const skapOppfolgingsplanOppgaver = (oppfolgingsdialoger: Oppfolgingsplan[] | undefined, sykmeldinger: Sykmelding[] | undefined, lenke: string): Oppgave[] => {
    ...
    if (planer.nyePlaner > 1) {
        leggTilOppgave(tekst('oppgaver.oppfoelgingsplan.sykmeldt.nyeplaner.flertall', {
    ...
    if (planer.avventendeGodkjenninger > 1) {
        leggTilOppgave(tekst('oppgaver.oppfoelgingsplan.avventendegodkjenninger.flertall', {
