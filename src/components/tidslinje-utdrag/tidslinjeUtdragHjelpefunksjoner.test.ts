...
describe('Tidslinjeutdrag hjelpefunksjoner', () => {
    ...
    it('Skal ikke vise noe dersom siste sykmeldings tom-dato ikke er passert, men sykmeldingen er avbrutt', () => {
        ...
        expect(
            skalViseUtdrag([passertSykmelding(), avbruttSykmelding])
        ).toEqual(false)
    })

    it('Skal vise noe dersom siste sykmeldings tom-dato ikke er passert', () => {
        expect(
            skalViseUtdrag([passertSykmelding(), aktivSykmelding()])
        ).toEqual(true)
    })

    it('Skal beregne antallDager', () => {
        const sykeforloepIDag: Sykeforloep[] = [
            { oppfolgingsdato: iDag.format('YYYY-MM-DD'), sykmeldinger: [] },
        ]
        expect(getSykefravaerVarighet(sykeforloepIDag, [])).toEqual(1)

        const sykeforloepFraForrigeDag: Sykeforloep[] = [
            {
                oppfolgingsdato: iDag.subtract(1, 'day').format('YYYY-MM-DD'),
                sykmeldinger: [],
            },
        ]
        expect(getSykefravaerVarighet(sykeforloepFraForrigeDag, [])).toEqual(2)

        const sykeforloepToUker: Sykeforloep[] = [
            {
                oppfolgingsdato: iDag.subtract(13, 'days').format('YYYY-MM-DD'),
                sykmeldinger: [],
            },
        ]
        expect(getSykefravaerVarighet(sykeforloepToUker, [])).toEqual(14)

        const sykeforloepFremITid: Sykeforloep[] = [
            {
                oppfolgingsdato: iDag.add(14, 'days').format('YYYY-MM-DD'),
                sykmeldinger: [],
            },
        ]
    ...
    it('Skal tvinge varighet til 275 dager hvis det er mindre enn 13 uker igjen til maksdato', () => {
        ...
        sykmelding.syketilfelleStartDato = iDag
            .subtract(merEnnTrettiniUker, 'days')
            .format('YYYY-MM-DD')
        sykmelding.sykmeldingsperioder[0].fom = iDag
            .subtract(merEnnTrettiniUker, 'days')
            .format('YYYY-MM-DD')
        ...
        const sykeforloep: Sykeforloep[] = [
            {
                ...
                sykmeldinger: [
                    {
                        fom: sykmelding.sykmeldingsperioder[0].fom,
                        tom: sykmelding.sykmeldingsperioder[0].tom,
                    },
                ],
            },
        ]
    ...
    it('Skal tvinge varighet til 272 når arbeidsrettet oppfølging er deaktivert, selv om beregning fra sykeforløp returnerer > 39 uker', () => {
        ...
        const sykeforloep: Sykeforloep[] = [
            {
                ...
                sykmeldinger: [
                    ...
                    {
                        fom: nySykmelding.sykmeldingsperioder[0].fom,
                        tom: nySykmelding.sykmeldingsperioder[0].tom,
                    },
                ],
            },
        ]
        ...
        expect(merEnnTrettiniUker).toBeGreaterThan(273)
        expect(
            getSykefravaerVarighet(sykeforloep, [sykmelding, nySykmelding])
        ).toEqual(272)
    })
    ...
    it('Skal beregne varighet fra sykeforløp når arbeidsrettet oppfølging er deaktivert og beregning fra sykeforløp returnerer < 39 uker', () => {
        ...
        const sykmelding = aktivSykmelding()
        sykmelding.syketilfelleStartDato = iDag
            .subtract(mindreEnnTrettiniUker, 'days')
            .format('YYYY-MM-DD')
        sykmelding.sykmeldingsperioder[0].fom = iDag
            .subtract(mindreEnnTrettiniUker, 'days')
            .format('YYYY-MM-DD')
    ...
    it('Nyeste sykeforloep velges', () => {
        ...
        expect(hentStartdatoFraSykeforloep(sykeforloep)).toEqual(
            iDag.startOf('day')
        )
        expect(hentStartdatoFraSykeforloep(sykeforloep.reverse())).toEqual(
            iDag.startOf('day')
        )
    })
    ...
    it('Nyeste sykeforloep velges selv når det er frem i tid', () => {
        ...
        expect(hentStartdatoFraSykeforloep(sykeforloep)).toEqual(
            planlagtDagForOperasjon.startOf('day')
        )
        expect(hentStartdatoFraSykeforloep(sykeforloep.reverse())).toEqual(
            planlagtDagForOperasjon.startOf('day')
        )
